...
### 2 预备知识[ ](https://dubbo.apache.org/zh/blog/2018/08/19/dubbo-现有心跳方案总结以及改进建议/#2-预备知识)
...
#### 2.2 心跳检测需要容错[ ](https://dubbo.apache.org/zh/blog/2018/08/19/dubbo-现有心跳方案总结以及改进建议/#22-心跳检测需要容错)

网络通信永远要考虑到最坏的情况，一次心跳失败，不能认定为连接不通，多次心跳失败，才能采取相应的措施。

#### 2.3 心跳检测不需要忙检测

忙检测的对立面是空闲检测，我们做心跳的初衷，是为了**保证连接的可用性**，以保证及时采取断连，重连等措施。如果一条通道上有频繁的 RPC 调用正在进行，我们不应该为通道增加负担去发送心跳包。**心跳扮演的角色应当是晴天收伞，雨天送伞。**
...
#### 3.4 定时任务二：处理重连和断连[ ](https://dubbo.apache.org/zh/blog/2018/08/19/dubbo-现有心跳方案总结以及改进建议/#34-定时任务二处理重连和断连)
...
类结构:class IdleStateHandler implements ChannelHandler 
...
从socketchannel获取pipeline,然后放入到pipeline里.
